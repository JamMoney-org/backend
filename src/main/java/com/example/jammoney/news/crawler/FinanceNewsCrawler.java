...
public class FinanceNewsCrawler {
    ...
    public List<NewsRequestDto> fetchTodayNews() {
        ...
        try {
            ...
            // Chrome 프로필/캐시를 디스크(/tmp)가 아닌 메모리(tmpfs, /dev/shm)로 유도해 I/O wait을 줄인다.
            try {
                Path shmProfileDir = Paths.get("/dev/shm/chrome-profile");
                Files.createDirectories(shmProfileDir);
                options.addArguments("--user-data-dir=" + shmProfileDir.toAbsolutePath());
                log.info("[Chrome 프로필 경로] {}", shmProfileDir);
            } catch (Exception e) {
                log.warn("[Chrome 프로필 경로 설정 실패] 기본값 사용 (/tmp)");
            }
